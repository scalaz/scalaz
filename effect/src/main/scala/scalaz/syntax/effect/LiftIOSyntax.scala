...
trait LiftIOOps[F[_],A] extends Ops[F[A]] {
...
trait LiftIOSyntax[F[_]]  { self => 
  implicit def ToLiftIOOps[A](v: F[A]): LiftIOOps[F, A] = new LiftIOOps[F,A] { def self = v; implicit def F: LiftIO[F] = self.F }
