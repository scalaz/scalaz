...
sealed trait ToMonadIOOps0 {
  implicit def ToMonadIOOpsUnapply[FA](v: FA)(implicit F0: Unapply[MonadIO, FA]): MonadIOOps[F0.M, F0.A] =
    new MonadIOOps[F0.M, F0.A](F0(v))(F0.TC)

}

trait ToMonadIOOps extends ToMonadIOOps0 with ToLiftIOOps with ToMonadOps {
  implicit def ToMonadIOOps[F[_], A](v: F[A])(implicit F0: MonadIO[F]): MonadIOOps[F, A] =
