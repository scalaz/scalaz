...
object HomomorphismLaws {
  ...
  def monoidIdentity[A, B, T](
    f: A => B
  )(assert: (B, B) => T)(implicit A: Monoid[A], B: Monoid[B]): T =
    assert(f(A.mempty), B.mempty)

  def semigroupAppend[A, B, T](
    fst: A,
    snd: A
  )(f: A => B)(assert: (B, B) => T)(implicit A: Semigroup[A], B: Semigroup[B]): T =
  ...
  def applyAp[F[_], G[_], A, B, T](
    transform: F ~> G
  )(fa: F[A], ff: F[A => B])(assert: (G[B], G[B]) => T)(implicit F: Apply[F], G: Apply[G]): T =
    assert(
  ...
  def applicativeIdentity[F[_], G[_], A, T](
    transform: F ~> G
  )(in: A)(assert: (G[A], G[A]) => T)(implicit F: Applicative[F], G: Applicative[G]): T =
    assert(
  ...
  def bindFlatMap[F[_], G[_], A, B, T](
    transform: F ~> G
  )(fa: F[A], f: A => F[B])(assert: (G[B], G[B]) => T)(implicit F: Bind[F], G: Bind[G]): T =
    assert(
  ...
  def monadIdentity[F[_], G[_], A, T](
    transform: F ~> G
  )(in: A)(assert: (G[A], G[A]) => T)(implicit F: Monad[F], G: Monad[G]): T =
  ...
  def cobindCoflatMap[F[_], G[_], A, B, T](
    transform: F ~> G
  )(fa: F[A], f: G[A] => B)(assert: (G[B], G[B]) => T)(implicit F: Cobind[F], G: Cobind[G]): T =
    assert(
  ...
  def comonadIdentity[F[_], G[_], A, T](
    transform: F ~> G
  )(in: F[A])(assert: (A, A) => T)(implicit F: Comonad[F], G: Comonad[G]): T =
  ...
  def categoryIdentity[F[_, _], G[_, _], A, T](
    transform: F ~~> G
  )(assert: (G[A, A], G[A, A]) => T)(implicit F: Category[F], G: Category[G]): T =
    assert(transform.of(F.id[A]), G.id[A])

  def strongFirst[F[_, _], G[_, _], A, B, C, T](
    transform: F ~~> G
  )(in: F[A, B])(assert: (G[A, B], G[A, B]) => T)(implicit F: Strong[F], G: Strong[G]): T = {
  ...
  def choiceLeft[F[_, _], G[_, _], A, B, C, T](
    transform: F ~~> G
  )(in: F[A, B])(assert: (G[A, B], G[A, B]) => T)(implicit F: Choice[F], G: Choice[G]): T = {
