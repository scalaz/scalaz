...
import java.util.concurrent.{ScheduledExecutorService, ConcurrentLinkedQueue, ExecutorService, TimeoutException, TimeUnit}
...
class Task[+A](val get: Future[Throwable \/ A]) {
   ...
   */
   def onFinish(f: Option[Throwable] => Task[Unit]): Task[A] = {
    ...
    val ttf = f.map(_.get.map(_ => ()))
    val res = this.get match {
      case Now(a) => Suspend(() => now(a), Some(ttf))
      case Async(listen) => Suspend(() => Async(listen), Some(ttf))
      case BindSuspend(thunk, g) =>
        Suspend(() => BindSuspend(thunk, g), Some(ttf))
      case BindAsync(listen, g) =>
        Suspend(() => BindAsync(listen, g), Some(ttf))
      case Suspend(thunk, Some(ff)) =>
        Suspend(thunk, Some(Kleisli(ff).flatMap(_ => Kleisli(ttf)).run))
      case Suspend(thunk, None) => Suspend(thunk, Some(ttf))
    }
    new Task(res)
  }
   ...
   */
  def unsafePerformSync: A = (this.get match {
    case Future.Now(a) => a
    case _ => {
      val latch = new java.util.concurrent.CountDownLatch(1)
      @volatile var result: Option[Throwable \/ A] = None
  ...
  def unsafePerformSyncAttempt: Throwable \/ A =
    try \/-(unsafePerformSync) catch { case t: Throwable => -\/(t) }
  ...
  def unsafePerformAsyncInterruptibly(f: (Throwable \/ A) => Unit, cancel: AtomicBoolean): Unit =
    unsafePerformListenInterruptibly(a => Trampoline.done(f(a)), cancel)
  ...
  def unsafePerformAsyncInterruptibly(f: (Throwable \/ A) => Unit) : () => Unit = {
    ...
    unsafePerformAsyncInterruptibly(r => a ! Some(r), completed)
    () => { a ! None }
  }
  ...
  def unsafePerformAsync(f: (Throwable \/ A) => Unit): Unit =
    unsafePerformListenInterruptibly(a => {
      Trampoline.done(f(a))
   ...
   * robust means of cancellation.
   */
  def unsafePerformListenInterruptibly(cb: Throwable \/ A => Trampoline[Unit], cancel: AtomicBoolean): Unit = {
    (this.get.stepInterruptiblyOnFinish(cancel, None)) match {
      case (Future.Now(a), andFinally) if ! cancel.get =>
        runAndFinally(cb, andFinally, a) 
      case (Future.Async(onFinish), andFinally) if !cancel.get =>
        onFinish(a => {
          if (!cancel.get) {
            runAndFinally(cb, andFinally, a)
            Trampoline.done(())
          } else {
            runAndFinally(cb, andFinally, -\/(Task.TaskInterrupted))
            Trampoline.done(())
          }
        })
      case (Future.BindAsync(onFinish, g), andFinally) if ! cancel.get =>
        onFinish(x =>
          Trampoline.delay((andFinally match {
            case Some(of) => new Task(g(x)).onFinish(of.map(f => new Task(f.map(_ => \/-(())))))
            case None => new Task(g(x)) 
  ...
  def unsafePerformListen(cb: Throwable \/ A => Trampoline[Unit]): Unit =
    (this.get.stepOnFinish(None): @unchecked) match {
      case (Future.Now(a), andFinally) =>
        runAndFinally(cb, andFinally, a) 
      case (Future.Async(onFinish), andFinally) =>
        onFinish(a => {
            runAndFinally(cb, andFinally, a)
            Trampoline.done(())
          })
      case (Future.BindAsync(onFinish, g), andFinally) =>
        onFinish(x =>
          Trampoline.delay((andFinally match {
            case Some(of) => new Task(g(x)).onFinish(of.map(f => new Task(f.map(_ => \/-(())))))
            case None => new Task(g(x)) 
  ...
  /* Runs the andFinally computation and if it throws an Exception, we thread that through.  The callbacks resume after the onFinish happens */ 
  private def runAndFinally[AA](cb: Throwable \/ AA => Trampoline[Unit], andFinally: Option[Option[Throwable] => Future[Unit]], a: (Throwable \/ AA)): Unit =
      andFinally match {
        case Some(f) => \/.fromTryCatchNonFatal(f(a.swap.toOption)) match {
          case \/-(ff) => 
            new Task(ff.map(_ => \/-(()))).unsafePerformListenInterruptibly(_ => Trampoline.done(cb(a)), new AtomicBoolean(false))
          case -\/(t) => cb(-\/(t)).run
        }
        case None =>
          cb(a).run
  ...
  def timed(timeoutInMillis: Long)(implicit scheduler: ScheduledExecutorService): Task[A] =
    new Task(Future.async[Throwable \/ A] { cb =>
      ...
      unsafePerformListenInterruptibly(a => Trampoline.done({
        if(done.compareAndSet(false, true)) 
          cb(a)
        else
