...
trait Apply[F[_]] extends Functor[F] { self =>
  ...
  @deprecated("given `F: Apply[F]` use `F.apply2(a,b)(f)` instead, or given `implicitly[Apply[F]]`, use `^(a,b)(f)`", "7")
  def map2[A, B, C](fa: => F[A], fb: => F[B])(f: (A, B) => C): F[C] =
    apply2(fa,fb)(f)

  @deprecated("given `F: Apply[F]` use `F.apply3(a,b,c)(f)` instead, or given `implicitly[Apply[F]]`, use `^^(a,b,c)(f)`", "7")
  def map3[A, B, C, D](fa: => F[A], fb: => F[B], fc: => F[C])(f: (A, B, C) => D): F[D] =
    apply3(fa,fb,fc)(f)

  @deprecated("given `F: Apply[F]` use `F.apply4(a,b,c,d)(f)` instead, or given `implicitly[Apply[F]]`, use `^^^(a,b,c,d)(f)`", "7")
