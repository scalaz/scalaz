...
trait Duals {
  implicit def DualUnpack[A]: Newtype[Dual[A], A] =
    Newtype.newtype(_.value, b => new Dual[A] {
      val value = b
    })

  implicit def DualZero[A: Zero]: Zero[Dual[A]] =
    Zero.zero(implicitly[Newtype[Dual[A], A]].pack(implicitly[Zero[A]].zero))

  implicit def DualSemigroup[A: Semigroup]: Semigroup[Dual[A]] = new Semigroup[Dual[A]] {
    def append(a1: Dual[A], a2: => Dual[A]) =
      implicitly[Newtype[Dual[A], A]].pack(implicitly[Semigroup[A]].append(a2.value, a1.value))
  }
