...
import Free._
import std.function.{function1Covariant => _, _}
import std.tuple._
...
object Free extends FreeFunctions with FreeInstances {
  ...
  private[scalaz] implicit def function1CovariantByName[T]: Monad[({type l[+a] = ((=> T) => a)})#l] with Zip[({type l[+a] = ((=> T) => a)})#l] with Unzip[({type l[+a] = ((=> T) => a)})#l] with Distributive[({type l[+a] = ((=> T) => a)})#l] = new Monad[({type l[+a] = ((=> T) => a)})#l] with Zip[({type l[+a] = ((=> T) => a)})#l] with Unzip[({type l[+a] = ((=> T) => a)})#l] with Distributive[({type l[+a] = ((=> T) => a)})#l] {
...
sealed abstract class Free[S[+_], +A](implicit S: Functor[S]) {
   ...
   * @since 7.0.5
   */
  final def foldRight[G[_], AA >: A](z: Id ~> G)(f: ({type λ[α] = S[G[α]]})#λ ~> G): G[AA] =
    this.resume match {
      case -\/(s) => f(S.map(s: S[Free[S, AA]])(_.foldRight(z)(f)))
      case \/-(r) => z(r: AA)
    }
  ...
  def duplicateF(implicit S: Functor[S]): Free[({type l[+a] = Free[S, a]})#l, A] =
    extendF[({type l[+a] = Free[S, a]})#l](NaturalTransformation.refl[({type l[a] = Free[S, a]})#l])

  /** Extension in `Free` as a comonad in the endofunctor category. */
  def extendF[T[+_]](f: ({type l[a] = Free[S, a]})#l ~> T)(implicit S: Functor[S], T: Functor[T]): Free[T, A] = mapSuspension(new (S ~> T) {
