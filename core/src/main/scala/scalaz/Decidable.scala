 ...
 */
////
trait Decidable[F[_]] extends Divisible[F] with CoapplicativeDecidable[F] { self =>
  ////
...
trait IsomorphismDecidable[F[_], G[_]] extends Decidable[F] with IsomorphismDivisible[F, G] with IsomorphismCoapplicativeDecidable[F, G]{
