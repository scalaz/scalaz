...
trait EitherInstances extends EitherInstances0 {
  ...
  implicit def LeftProjectionEIso2[E]: ({type λ[α] = LeftProjection[E, α]})#λ <~> ({type λ[α] = Either[E, α]})#λ = new IsoFunctorTemplate[({type λ[α] = LeftProjection[E, α]})#λ, ({type λ[α] = Either[E, α]})#λ] {
    def to[A](fa: LeftProjection[E, A]) = fa.e
    def from[A](ga: Either[E, A]) = ga.left
  }

  /** [[scala.Either.LeftProjection]] is isomorphic to [[scala.Either]], when the type parameter `E` is partially applied. */
  implicit def FirstLeftProjectionEIso2[E]: ({type λ[α] = LeftProjection[E, α] @@ First})#λ <~> ({type λ[α] = Either[E, α]})#λ = new IsoFunctorTemplate[({type λ[α] = LeftProjection[E, α] @@ First})#λ, ({type λ[α] = Either[E, α]})#λ] {
    def to[A](fa: LeftProjection[E, A] @@ First) = fa.e
    def from[A](ga: Either[E, A]) = First(ga.left)
  }

  /** [[scala.Either.LeftProjection]] is isomorphic to [[scala.Either]], when the type parameter `E` is partially applied. */
  implicit def LastLeftProjectionEIso2[E]: ({type λ[α] = LeftProjection[E, α] @@ Last})#λ <~> ({type λ[α] = Either[E, α]})#λ = new IsoFunctorTemplate[({type λ[α] = LeftProjection[E, α] @@ Last})#λ, ({type λ[α] = Either[E, α]})#λ] {
  ...
  implicit def RightProjectionAIso2[A]: ({type λ[α] = RightProjection[α, A]})#λ <~> ({type λ[α] = Either[α, A]})#λ = new IsoFunctorTemplate[({type λ[α] = RightProjection[α, A]})#λ, ({type λ[α] = Either[α, A]})#λ] {
    def to[E](fa: RightProjection[E, A]) = fa.e
    def from[E](ga: Either[E, A]) = ga.right
  }

  /** [[scala.Either.RightProjection]] is isomorphic to [[scala.Either]], when the type parameter `A` is partially applied. */
  implicit def FirstRightProjectionAIso2[A]: ({type λ[α] = RightProjection[α, A] @@ First})#λ <~> ({type λ[α] = Either[α, A]})#λ = new IsoFunctorTemplate[({type λ[α] = RightProjection[α, A] @@ First})#λ, ({type λ[α] = Either[α, A]})#λ] {
    def to[E](fa: RightProjection[E, A] @@ First) = fa.e
    def from[E](ga: Either[E, A]) = First(ga.right)
  }

  /** [[scala.Either.RightProjection]] is isomorphic to [[scala.Either]], when the type parameter `A` is partially applied. */
  implicit def LastRightProjectionAIso2[A]: ({type λ[α] = RightProjection[α, A] @@ Last})#λ <~> ({type λ[α] = Either[α, A]})#λ = new IsoFunctorTemplate[({type λ[α] = RightProjection[α, A] @@ Last})#λ, ({type λ[α] = Either[α, A]})#λ] {
