...
sealed trait ToChoiceOpsU[TC[F[_, _]] <: Choice[F]] {
  implicit def ToChoiceOpsUnapply[FA](v: FA)(implicit F0: Unapply2[TC, FA]): ChoiceOps[F0.M, F0.A, F0.B] =
    new ChoiceOps[F0.M, F0.A, F0.B](F0(v))(using F0.TC)

}

trait ToChoiceOps0[TC[F[_, _]] <: Choice[F]] extends ToChoiceOpsU[TC] {

  implicit def ToChoiceOps[F[_, _],A, B](v: F[A, B])(implicit F0: TC[F]): ChoiceOps[F, A, B] =
  ...
  implicit def ToChoiceVFromKleisliLike[G[_], F[G[_], _, _],A, B](v: F[G, A, B])(implicit F0: TC[F[G, *, *]]): ChoiceOps[F[G, *, *], A, B] =
    new ChoiceOps[F[G, *, *], A, B](v)(using F0)
...
trait ChoiceSyntax[F[_, _]] extends CategorySyntax[F] {
  implicit def ToChoiceOps[A, B](v: F[A, B]): ChoiceOps[F, A, B] = new ChoiceOps[F, A, B](v)(using ChoiceSyntax.this.F)
