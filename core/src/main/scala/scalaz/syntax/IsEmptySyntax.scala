...
sealed trait ToIsEmptyOps0 {
  implicit def ToIsEmptyOpsUnapply[FA](v: FA)(implicit F0: Unapply[IsEmpty, FA]): IsEmptyOps[F0.M, F0.A] =
    new IsEmptyOps[F0.M, F0.A](F0(v))(F0.TC)

}

trait ToIsEmptyOps extends ToIsEmptyOps0 with ToPlusEmptyOps {
  implicit def ToIsEmptyOps[F[_], A](v: F[A])(implicit F0: IsEmpty[F]): IsEmptyOps[F, A] =
