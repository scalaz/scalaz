...
trait PlusEmptySyntax[F[_]] extends PlusSyntax[F] { self => 
  implicit def ToPlusEmptyOps[A](v: F[A]): PlusEmptyOps[F, A] = new PlusEmptyOps[F,A] { def self = v; implicit def F: PlusEmpty[F] = self.F }
