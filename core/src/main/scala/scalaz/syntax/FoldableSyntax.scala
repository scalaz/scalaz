...
final class FoldableOps[F[_],A] private[syntax](val self: F[A])(implicit val F: Foldable[F]) extends Ops[F[A]] {
  ...
  final def foldRightMTrampoline[G[_]: Monad: Traverse, B](z: => B)(f: (A, => B) => G[B]): G[B] = F.foldRightMTrampoline(self, z)(f)
  final def foldLeftMTrampoline[G[_]: Monad: Traverse, B](z: B)(f: (B, A) => G[B]): G[B] = F.foldLeftMTrampoline(self, z)(f)
