...
final class FoldableOps[F[_],A] private[syntax](val self: F[A])(implicit val F: Foldable[F]) extends Ops[F[A]] {
  ...
  final def fold1Opt(implicit A: Semigroup[A]): Option[A] = foldMap1Opt[A]()
  ...
  final def sumr1Opt(implicit A: Semigroup[A]): Option[A] = F.foldRight1Opt(self)(A.append(_,_))
  final def suml(implicit A: Monoid[A]): A = F.foldLeft(self, A.zero)(A.append(_, _))
  final def suml1Opt(implicit A: Semigroup[A]): Option[A] = F.foldLeft1Opt(self)(A.append(_,_))
  ...
  final def splitBy[B: Equal](f: A => B): IList[(B, NonEmptyList[A])] =
    F.foldRight(self, IList.empty[(B, NonEmptyList[A])]){ (a, bas) =>
      ...
      bas match {
        case INil() =>
          IList.single((fa, NonEmptyList.nel(a, Nil)))
        case ICons((b, as), tail) =>
          if (Equal[B].equal(fa, b)) ICons((b, a <:: as), tail)
          else ICons((fa, NonEmptyList.nel(a, Nil)), bas)
      }
    }
  final def splitByRelation(r: (A, A) => Boolean): IList[NonEmptyList[A]] =
    F.foldRight(self, IList.empty[NonEmptyList[A]]){ (a, neas) =>
      neas match {
        case INil() =>
          IList.single(NonEmptyList.nel(a, Nil))
        case ICons(nea, tail) =>
          if (r(a, nea.head)) ICons(a <:: nea, tail)
          else ICons(NonEmptyList.nel(a, Nil), neas)
      }
    }
  ...
  final def msumlU(implicit G: Unapply[PlusEmpty, A]): G.M[G.A] = msuml[G.M, G.A](G.leibniz, G.TC)
  ////
}
