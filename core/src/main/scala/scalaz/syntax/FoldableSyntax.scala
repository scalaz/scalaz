...
final class FoldableOps[F[_],A] private[syntax](val self: F[A])(implicit val F: Foldable[F]) extends Ops[F[A]] {
  ...
  final def distinctBy[B: Equal](f: A => B): IList[A] = F.distinctE(self)(Equal.equalBy(f))
...
sealed trait ToFoldableOps0 {
  implicit def ToFoldableOpsUnapply[FA](v: FA)(implicit F0: Unapply[Foldable, FA]): FoldableOps[F0.M, F0.A] =
