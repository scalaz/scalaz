...
trait FoldableOps[F[_],A] extends Ops[F[A]] {
  ...
  final def foldRight1Opt(f: (A, => A) => A): Option[A] = foldRight(None: Option[A])((a, optA) => optA map (aa => f(a, aa)) orElse Some(a))
  final def foldLeft[B](z: B)(f: (B, A) => B): B = F.foldLeft(self, z)(f)
  final def foldLeft1Opt(f: (A, A) => A): Option[A] = foldLeft(None: Option[A])((optA, a) => optA map (aa => f(aa, a)) orElse Some(a))
  ...
  final def foldr1Opt[B](f: A => (=> A) => A): Option[A] = foldr(None: Option[A])(a => optA => optA map (aa => f(a)(aa)) orElse Some(a))
  final def foldl[B](z: B)(f: B => A => B): B = F.foldl(self, z)(f)
  final def foldl1Opt[B](f: A => A => A): Option[A] = foldl(None: Option[A])(optA => a => optA map (aa => f(aa)(a)) orElse Some(a))
