...
final class FoldableOps[F[_],A] private[syntax](val self: F[A])(implicit val F: Foldable[F]) extends Ops[F[A]] {
  ...
  final def foldMap1Maybe[B: Semigroup](f: A => B = (a: A) => a): Maybe[B] = F.foldMap1Maybe(self)(f)
  final def foldRight[B](z: => B)(f: (A, => B) => B): B = F.foldRight(self, z)(f)
  final def foldMapRight1Maybe[B](z: A => B)(f: (A, => B) => B): Maybe[B] = F.foldMapRight1Maybe(self)(z)(f)
  final def foldRight1Maybe(f: (A, => A) => A): Maybe[A] = F.foldRight1Maybe(self)(f)
  final def foldLeft[B](z: B)(f: (B, A) => B): B = F.foldLeft(self, z)(f)
  final def foldMapLeft1Maybe[B](z: A => B)(f: (B, A) => B): Maybe[B] = F.foldMapLeft1Maybe(self)(z)(f)
  final def foldLeft1Maybe(f: (A, A) => A): Maybe[A] = F.foldLeft1Maybe(self)(f)
  ...
  final def findMapM[G[_] : Monad, B](f: A => G[Maybe[B]]): G[Maybe[B]] = F.findMapM(self)(f)
  final def findLeft(f: A => Boolean): Maybe[A] = F.findLeft(self)(f)
  final def findRight(f: A => Boolean): Maybe[A] = F.findRight(self)(f)
  final def fold(implicit A: Monoid[A]): A = F.fold(self)
  final def fold1Maybe(implicit A: Semigroup[A]): Maybe[A] = F.fold1Maybe(self)
  final def foldr[B](z: => B)(f: A => (=> B) => B): B = F.foldr(self, z)(f)
  final def foldr1Maybe(f: A => (=> A) => A): Maybe[A] = F.foldr1Maybe(self)(f)
  final def foldl[B](z: B)(f: B => A => B): B = F.foldl(self, z)(f)
  final def foldl1Maybe(f: A => A => A): Maybe[A] = F.foldl1Maybe(self)(f)
  ...
  final def index(n: Int): Maybe[A] = F.index(self, n)
  final def indexOr(default: => A, n: Int): A = F.indexOr(self, default, n)
  final def sumr(implicit A: Monoid[A]): A = F.sumr(self)
  final def sumr1Maybe(implicit A: Semigroup[A]): Maybe[A] = F.sumr1Maybe(self)
  final def suml(implicit A: Monoid[A]): A = F.suml(self)
  final def suml1Maybe(implicit A: Semigroup[A]): Maybe[A] = F.suml1Maybe(self)
  ...
  final def maximum(implicit A: Order[A]): Maybe[A] = F.maximum(self)
  final def maximumOf[B: Order](f: A => B): Maybe[B] = F.maximumOf(self)(f)
  final def maximumBy[B: Order](f: A => B): Maybe[A] = F.maximumBy(self)(f)
  final def minimum(implicit A: Order[A]): Maybe[A] = F.minimum(self)
  final def minimumOf[B: Order](f: A => B): Maybe[B] = F.minimumOf(self)(f)
  final def minimumBy[B: Order](f: A => B): Maybe[A] = F.minimumBy(self)(f)
  final def extrema(implicit A: Order[A]): Maybe[(A, A)] = F.extrema(self)
  final def extremaOf[B: Order](f: A => B): Maybe[(B, B)] = F.extremaOf(self)(f)
  final def extremaBy[B: Order](f: A => B): Maybe[(A, A)] = F.extremaBy(self)(f)
  ...
  final def splitWith(p: A => Boolean): IList[NonEmptyList[A]] = F.splitWith(self)(p)
  ...
  final def selectSplit(p: A => Boolean): IList[NonEmptyList[A]] = F.selectSplit(self)(p)
