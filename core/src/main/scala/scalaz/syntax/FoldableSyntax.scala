...
final class FoldableOps[F[_],A] private[syntax](val self: F[A])(implicit val F: Foldable[F]) extends Ops[F[A]] {
  ...
  final def asum[G[_], B](implicit ev: A === G[B], G: Alternative[G]): G[B] = F.asum(ev.subst[F](self))(G)
  final def atraverse[G[_], B](f: A => G[B])(implicit G: Alternative[G]): G[B] = F.atraverse(self)(f)(G)
  ////
}
