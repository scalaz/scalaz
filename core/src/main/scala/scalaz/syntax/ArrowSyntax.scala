...
trait ArrowSyntax[F[_, _]] extends CategorySyntax[F] { self =>
  implicit def ToArrowOps[A, B](v: F[A, B]): ArrowOps[F, A, B] = new ArrowOps[F, A, B] { def self = v; implicit def F: Arrow[F] = self.F }
