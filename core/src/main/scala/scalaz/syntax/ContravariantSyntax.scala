...
trait ContravariantSyntax[F[_]]  {
  implicit def ToContravariantOps[A](v: F[A])(implicit F0: Contravariant[F]): ContravariantOps[F, A] = new ContravariantOps[F,A] { def self = v; implicit def F: Contravariant[F] = F0 }
