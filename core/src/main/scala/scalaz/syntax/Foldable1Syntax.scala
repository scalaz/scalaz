...
final class Foldable1Ops[F[_],A] private[syntax](val self: F[A])(implicit val F: Foldable1[F]) extends Ops[F[A]] {
  ...
  final def asum1[G[_], B](implicit ev: A === G[B], G: Plus[G]): G[B] = F.asum1(ev.subst[F](self))
  final def atraverse1[G[_], B](f: A => G[B])(implicit G: Plus[G]): G[B] = F.atraverse1(self)(f)(G)
