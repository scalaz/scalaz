...
trait CobindSyntax[F[_]] extends FunctorSyntax[F] { self => 
  implicit def ToCobindOps[A](v: F[A]): CobindOps[F, A] = new CobindOps[F,A] { def self = v; implicit def F: Cobind[F] = self.F }
