...
final class OptionOps2[A](self: Option[A]) {
  ...
  final class Fold[X] private[OptionOps2] (s: A => X) {
    def none(n: => X): X = o.cata(self)(s, n)
  }
  ...
  final class Conditional[X] private[OptionOps2] (s: => X) {
...
trait ToOptionOps {
  ...
  implicit def ToOptionOps2FromOption[A](a: Option[A]): OptionOps2[A] = new OptionOps2(a)
}
