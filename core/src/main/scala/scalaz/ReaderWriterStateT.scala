...
sealed trait ReaderWriterStateT[F[_], R, W, S, A] {
  ...
  def map[B](f: A => B)(implicit F: Functor[F]): ReaderWriterStateT[F, R, W, S, B] = new ReaderWriterStateT[F, R, W, S, B] {
    def run(r: R, s: S): F[(W, B, S)] = F.map(self.run(r, s)) {
      case (w, a, s) => (w, f(a), s)
    }
  }

  def flatMap[B](f: A => ReaderWriterStateT[F, R, W, S, B])(implicit F: Bind[F], W: Semigroup[W]): ReaderWriterStateT[F, R, W, S, B] = new ReaderWriterStateT[F, R, W, S, B] {
    def run(r: R, s: S): F[(W, B, S)] = {
      F.bind(self.run(r, s)) {
        case (w1, a, s1) => {
          F.map(f(a).run(r, s1)) {
