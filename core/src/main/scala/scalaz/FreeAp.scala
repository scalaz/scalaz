...
sealed abstract class FreeAp[F[_],A] {
  ...
  // inferred kinds of the type arguments (F,B) do not conform to the expected kinds of the type parameters (type S,type A).
  // F's type parameters do not match type S's expected parameters: type _ is invariant, but type _ is declared covariant
  /*
