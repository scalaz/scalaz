...
sealed trait LazyEitherT[A, F[_], B] {
  ...
  def run(implicit i: F =~~= Identity): LazyEither[A, B] =
    runT
...
trait LazyEitherTs {
  ...
  sealed trait LazyLeftProjectionT[A, F[_], B] {
    ...
    def toLazyOption(implicit i: F =~~= Identity): LazyOption[A] =
