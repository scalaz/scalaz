...
private trait LeftFunctor[F[_,_], X] extends Functor[({type λ[α] = F[α, X]})#λ] {
...
private trait LeftFoldable[F[_,_], X] extends Foldable[({type λ[α] = F[α, X]})#λ] {
...
private trait RightFoldable[F[_,_], X] extends Foldable[({type λ[α] = F[X, α]})#λ] {
  implicit def F: Bifoldable[F]
...
private trait UFoldable[F[_,_]] extends Foldable[({type λ[α] = F[α, α]})#λ] {
  implicit def F: Bifoldable[F]
...
private trait LeftTraverse[F[_,_], X] extends Traverse[({type λ[α] = F[α, X]})#λ]
...
private trait RightTraverse[F[_,_], X] extends Traverse[({type λ[α] = F[X, α]})#λ]
...
private trait UTraverse[F[_,_]] extends Traverse[({type λ[α] = F[α, α]})#λ]
