...
sealed abstract class NullResultInstances extends NullResultInstances0 {
  ...
  implicit val nullResultArrow: Arrow[NullResult] with Choice[NullResult] with ProChoice[NullResult] = new Arrow[NullResult] with Choice[NullResult] with ProChoice[NullResult] {
    def id[A] =
