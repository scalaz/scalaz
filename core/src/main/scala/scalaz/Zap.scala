...
trait ZapInstances {
  ...
  implicit def productCoproductZap[F[_], FF[_], G[_], GG[_]](implicit d1: Zap[F, FF], d2: Zap[G, GG]):
    Zap[({ type λ[α] = Either[F[α], G[α]] })#λ, ({ type λ[α] = (FF[α], GG[α]) })#λ] =
      new Zap[({ type λ[α] = Either[F[α], G[α]] })#λ, ({ type λ[α] = (FF[α], GG[α]) })#λ] {
        def zapWith[A, B, C](a: Either[F[A], G[A]], b: (FF[B], GG[B]))(f: (A, B) => C) =
          a match {
            case Left(fa) => d1.zapWith(fa, b._1)(f)
            case Right(ga) => d2.zapWith(ga, b._2)(f)
          }
      }
  ...
  implicit def coproductProductZap[F[_], FF[_], G[_], GG[_]](implicit d1: Zap[FF, F], d2: Zap[GG, G]):
    Zap[({ type λ[α] = (FF[α], GG[α]) })#λ, ({ type λ[α] = Either[F[α], G[α]] })#λ] =
      new Zap[({ type λ[α] = (FF[α], GG[α]) })#λ, ({ type λ[α] = Either[F[α], G[α]] })#λ] {
        def zapWith[A, B, C](a: (FF[A], GG[A]), b: Either[F[B], G[B]])(f: (A, B) => C) =
          b match {
            case Left(fb) => d1.zapWith(a._1, fb)(f)
            case Right(gb) => d2.zapWith(a._2, gb)(f)
          }
      }
      ...
      new Zap[({type λ[α] = Free[F, α]})#λ, ({type λ[α] = Cofree[G, α]})#λ] {
        def zapWith[A, B, C](ma: Free[F, A], wb: Cofree[G, B])(f: (A, B) => C): C =
          ma.resume match {
            ...
            case Left(k) => d.zapWith(k, wb.tail)(zapWith(_, _)(f))
          }
      }
      ...
      new Zap[({type λ[α] = Cofree[F, α]})#λ, ({type λ[α] = Free[G, α]})#λ] {
        def zapWith[A, B, C](wa: Cofree[F, A], mb: Free[G, B])(f: (A, B) => C): C =
          mb.resume match {
            ...
            case Left(k) => d.zapWith(wa.tail, k)(zapWith(_, _)(f))
          }
      }
}
