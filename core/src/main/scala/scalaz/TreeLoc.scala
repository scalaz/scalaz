...
sealed abstract class TreeLocInstances {
  implicit val treeLocInstance: Comonad[TreeLoc] with Traverse1[TreeLoc] = new Comonad[TreeLoc] with Traverse1[TreeLoc] {
    ...
    private[this] val ForestT1: Traverse1[({type l[a] = OneAnd[Stream, Tree[a]]})#l] =
      Traverse1[({type l[a] = OneAnd[Stream, a]})#l].compose[Tree]
    ...
    private[this] val ParentsT1: Traverse1[({type l[a] = OneAnd[Stream, Parent[a]]})#l] =
      Traverse1[({type l[a] = OneAnd[Stream, a]})#l].compose[Parent]
  }
