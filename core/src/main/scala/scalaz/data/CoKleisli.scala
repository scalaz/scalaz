...
trait CoKleisli[A, W[_], B] {
  ...
  def *->* : (({type λ[α] = CoKleisli[A, W, α]})#λ *->* B) =
    data.*->*.**->**[({type λ[α] = CoKleisli[A, W, α]})#λ, B](this)

  def *->*->* : *->*->*[A, ({type λ[α, β] = CoKleisli[α, W, β]})#λ, B] =
    data.*->*->*.**->**->**[A, ({type λ[α, β] = CoKleisli[α, W, β]})#λ, B](this)
  ...
  def redaer(implicit i: Ident[A] =:= W[A]): A => B =
    a => run(ident(a))
...
trait CoKleislis {
  ...
  type Redaer[A, B] = CoKleisli[A, Ident, B]
  ...
  def redaer[A, B](r: A => B): Redaer[A, B] =
    coKleisli[A, Ident, B](a => r(a.value))
