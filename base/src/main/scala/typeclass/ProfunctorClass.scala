...
object ProfunctorClass {
  trait LeftRightMap[F[_, _]] extends Alt[LeftRightMap[F]] with Profunctor[F] { self: Profunctor[F] =>
  ...
  trait Dimap[F[_, _]] extends Alt[Dimap[F]] with Profunctor[F] { self: Profunctor[F] =>
