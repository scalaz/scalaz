...
trait MonadInstances {
  implicit val option: Monad[Option] = new MonadClass.Template[Option] with Bind.Flatten[Option] {
  ...
  implicit val list: Monad[List] = new MonadClass.Template[List] with Bind.Flatten[List] {
