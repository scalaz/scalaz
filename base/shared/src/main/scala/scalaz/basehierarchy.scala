...
object BaseHierarchy {
  ...
  trait BH0 extends BH1 {
    ...
    implicit def categorySemicategory[=>:[_, _]](implicit C: Category[=>:]): Semicategory[=>:] =
      instanceOf(C)
    implicit def comonadCobind[F[_]](implicit F: Comonad[F]): Cobind[F] = instanceOf(F)
    implicit def functorInvariantFunctor[F[_]](implicit F: Functor[F]): InvariantFunctor[F] =
      instanceOf(F)
  }

  trait BH1 extends BH2 {
    implicit def contravariantInvariantFunctor[F[_]](
      implicit F: Contravariant[F]
    ): InvariantFunctor[F]                                                        = instanceOf(F)
