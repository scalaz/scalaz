...
object ChoiceClass {
  implicit val functionChoice: Choice[Function1] = instanceOf(
    new ChoiceClass[Function1] {
...
trait ChoiceSyntax {
  implicit final class ToChoiceOps[P[_, _], A, B](self: P[A, B]) {
    def leftchoice[C](implicit ev: Choice[P]): P[A \/ C, B \/ C] = macro ops.Ops.i_0
    def rightchoice[C](implicit ev: Choice[P]): P[C \/ A, C \/ B] = macro ops.Ops.i_0
  }
}
