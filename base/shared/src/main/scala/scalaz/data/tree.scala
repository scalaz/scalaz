...
private[data] object FixFreeImpl extends FixFreeModule {
  ...
  override def monad[F[_]: Functor]: Monad[FixFree[F, ?]] =
    instanceOf(
      new MonadClass[FixFree[F, ?]] with MonadClass.DeriveMap[FixFree[F, ?]] with BindClass.DeriveFlatten[FixFree[F, ?]]
      with BindClass.DeriveAp[FixFree[F, ?]] {
        def pure[A](a: A): FixFree[F, A] = FixFreeImpl.pure(a)
        def flatMap[A, B](fa: FixFree[F, A])(f: A => FixFree[F, B]): FixFree[F, B] =
