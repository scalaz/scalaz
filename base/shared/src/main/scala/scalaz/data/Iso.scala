...
import Prelude._
import scalaz.typeclass.{Contravariant, Functor, InvariantFunctor}
...
object Iso {
  ...
  def unsafeTotal[A, B](ab: A => B, ba: B => A): Iso[A, B] = new Iso[A, B] {
    def to(a: A): B = ab(a)
    def from(b: B): A = ba(b)
  }
  ...
  object Product {
    ...
    final def associate[A, B, C]: Iso[A ⨂ (B ⨂ C), (A ⨂ B) ⨂ C] = unsafeTotal(
      { case (a, (b, c)) => ((a, b), c) },
      { case ((a, b), c) => (a, (b, c)) })

    final def commute[A, B]: Iso[A ⨂ B, B ⨂ A] = unsafeTotal(
      { case (a, b) => (b, a) },
      { case (b, a) => (a, b) })

    final def unit[A]: Iso[A, A ⨂ Id] = unsafeTotal(
      { case a => (a, ()) },
      { case (a, ()) => a })
