...
sealed abstract class Freer[F[_], A] {
  ...
  final def foldMap[M[_]](α: F ~> M)(implicit M: Monad[M]): M[A] =
    step match {
      case Freer.Pure(a)      => M.pure(a)
      case Freer.LiftF(fa)    => Forall.specialize[λ[α => F[α] => M[α]], A](α).apply(fa)
      case Freer.Impure(f, q) => M.flatMap(f foldMap α)(a => Freer.runQuiver(f)(q).foldMap(α))
    }

  @tailrec
  final def runFreer(implicit F: Functor[F]): F[Freer[F, A]] \/ A =
    this match {
      case Freer.Pure(a)   => \/-(a)
...
object Freer extends FreerInstances {
  ...
  private[Freer] def runQuiver[F[_], A, B](fa: Freer[F, A])(q: Freer.Quiver[F, A, B]): Freer[F, B] =
    q.foldLeft[Freer[F, ?]](fa)(
      ν[RightAction[Freer[F, ?], =>:[F, ?, ?]]][A, B](
        (f, k) => f.flatMap(Kleisli.runKleisli(k))
      )
    )
}
