...
trait TheseInstances {
  ...
  implicit final def theseMonad[L: Semigroup]: Monad[These[L, ?]] = instanceOf(
  ...
  implicit final def theseTraversable[L]: Traversable[These[L, ?]] = instanceOf(
    new TraversableClass.DeriveSequence[These[L, ?]] with FoldableClass.DeriveToList[These[L, ?]] {
  ...
  implicit final def theseSemigroup[L: Semigroup, R: Semigroup]: Semigroup[These[L, R]] = instanceOf[SemigroupClass[These[L, R]]](
    _.append(_)
  )

  implicit final def theseDebug[L, R](implicit L: Debug[L], R: Debug[R]): Debug[These[L, R]] = instanceOf[DebugClass[These[L, R]]](
