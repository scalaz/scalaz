...
object KleisliTest extends SpecLite {
  ...
  object instances {
    ...
    def plus[F[_] : MonadPlus, A] = Plus[({type f[a] = Kleisli[F, A, a]})#f]
    def empty[F[_] : MonadPlus, A] = PlusEmpty[({type f[a] = Kleisli[F, A, a]})#f]
    def profunctor[F[_]: Apply] = Profunctor[({type λ[α, β]=Kleisli[F, α, β]})#λ]
    def profunctor[F[_]: Applicative] = Profunctor[({type λ[α, β]=Kleisli[F, α, β]})#λ]
    def profunctor[F[_]: Bind] = Profunctor[({type λ[α, β]=Kleisli[F, α, β]})#λ]
