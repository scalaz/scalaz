...
object EnumeratorTTest extends SpecLite {
  ...
  "eof" in {
    val enum1 = enumEofT[Int, Id]
    (consume[Int, Id, List] &= enum1).run must_===(Nil)
  }

  "map" in {
    val enum1 = enumStream[Int, Id](Stream(1, 2, 3))
    (consume[Int, Id, List] &= enum1.map(_ * 2)).run must_===(List(2, 4, 6))
  }

  "flatMap" in {
    val enum1 = enumStream[Int, Id](Stream(1, 2, 3))
    (consume[Int, Id, List] &= enum1.flatMap(i => enum1.map(_ + i))).run must_===(List(2, 3, 4, 3, 4, 5, 4, 5, 6))
  }

  "flatten in a generalized fashion" in {
    val enum1 = enumOne[List[Int], List](List(1, 2, 3))
    (consume[Int, List, List] &= enum1.flatten).run.flatten must_===(List(1, 2, 3))
  }

  "uniq" in {
    val enum1 = enumStream[Int, Id](Stream(1, 1, 2, 2, 2, 3, 3))
    (consume[Int, Id, List] &= enum1.uniq).run must_===(List(1, 2, 3))
  }

  "zipWithIndex" in {
    val enum1 = enumStream[Int, Id](Stream(3, 4, 5))
    (consume[(Int, Long), Id, List] &= enum1.zipWithIndex).run must_===(List((3, 0L), (4, 1L), (5, 2L)))
  }
  ...
  "zipWithIndex in combination with another function" in {
    val enum1 = enumStream[Int, Id](Stream(3, 4, 4, 5))
    (consume[(Int, Long), Id, List] &= enum1.uniq.zipWithIndex).run must_===(List((3, 0L), (4, 1L), (5, 2L)))
  }

  "lift" in {
    val enum1 = EnumeratorT.enumeratorTMonadTrans.liftM(List(1, 2, 3))
    (collectT[Int, List, Id] &= enum1.map(_ * 2)).run must_===(List(2, 4, 6))
  }

  "enumerate an array" in {
    val enum1 = enumArray[Int, Id](Array(1, 2, 3, 4, 5), 0, Some(3))
    (consume[Int, Id, List] &= enum1).run must_===(List(1, 2, 3))
  }

  "allow for nesting of monads" in {
    type OIO[α] = OptionT[IO, α]
    val enum1 = enumIterator[Int, OIO](List(1, 2, 3).iterator)
    (consume[Int, OIO, List] &= enum1.map(_ * 2)).run.run.unsafePerformIO() must_===(Some(List(2, 4, 6)))
  }

  "drain" in {
    val enum1 = enumStream[Int, Id](Stream(1, 2, 3))
    enum1.drainTo[List] must_===(List(1, 2, 3))
  }
