...
class RopeTest extends Specification with Sugar with ScalaCheck {
  ...
  "converting an array gives a rope of the same length" verifies {
    (array: Array[Int]) =>
      Rope.fromArray(array).length ≟ array.length
  }

  "indexing a rope converted from an array is the same as indexing this array" verifies {
    (array: Array[Int], i: Int) =>
  ...
  "length of a rope is the same as its length as a stream" verifies {
    (rope: Rope[Int]) =>
      rope.length ≟ rope.toStream.length
  }

  "indexing a rope is the same as converting it to a stream and indexing that" verifies {
    (rope: Rope[Int], i: Int) =>
      if (i >= 0 && i < rope.length) (rope(i) must (beEqualTo(rope.toStream(i))))
      else (rope(i) must throwA[RuntimeException])
  }

  "building a rope from chunks and converting it back is the same as filtering out empty chunks" verifies {
    (chunks: List[ImmutableArray[Int]]) =>
      Rope.fromChunks(chunks).chunks must beTheSameSeqAs(chunks.filterNot(_.isEmpty))
  }

  "a rope converted to a stream is the same sequence as the original rope" verifies {
    (rope: Rope[Int]) =>
  ...
  "appending ropes works correctly" verifies {
    (rope1: Rope[Int], rope2: Rope[Int]) =>
