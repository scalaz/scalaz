...
object IterateeUsage extends App {
  ...
  ((peek[Unit, Int, Id] >>== Stream(1, 2, 3)) apply(_ => none)) assert_=== Some(1)
  ((head[Unit, Int, Id] >>== Stream()) apply(_ => some(0))) assert_=== None

  ((head[Unit, Int, IO] >>== Iterator(1, 2, 3)) apply(_ => IO(none)) unsafePerformIO) assert_=== Some(1)
  ((length[Unit, Int, IO] >>== Iterator(10, 20, 30)) apply(_ => IO(-1)) unsafePerformIO) assert_=== 3
  ((peek[Unit, Int, IO] >>== Iterator(1, 2, 3)) apply(_ => IO(none)) unsafePerformIO) assert_=== Some(1)
  ((head[Unit, Int, IO] >>== Iterator()) apply(_ => IO(Some(-1))) unsafePerformIO) assert_=== None

  ((take[Unit, Int, List](3) >>== (1 to 10).toStream) apply(_ => List())) assert_=== List(1, 2, 3)
  ((takeWhile[Unit, Int, List](_ <= 5) >>== (1 to 10).toStream) apply(_ => List())) assert_=== (1 to 5).toList
  ((takeUntil[Unit, Int, List](_ > 5) >>== (1 to 10).toStream) apply(_ => List())) assert_=== (1 to 5).toList

  val readLn = takeWhile[Unit, Char, List](_ != '\n') flatMap (ln => drop[Unit, Char, Id](1).map(_ => ln))
  ((collect[Unit, List[Char], List] %= readLn.sequenceI[List[List[Char]]] >>== "Iteratees\nare\ncomposable".toStream) apply(_ => List())) assert_=== List("Iteratees".toList, "are".toList, "composable".toList)

  ((collect[Unit, List[Int], List] %= splitOn(_ % 3 != 0) >>== (1 to 10).toStream) apply(_ => List())) assert_=== List(List(1, 2), List(4, 5), List(7, 8), List(10))

  ((collect[Unit, Int, List] %= map((_:String).toInt) >>== Stream("1", "2", "3")) apply(_ => List())) assert_=== List(1, 2, 3)
  ((collect[Unit, Int, List] %= filter((_:Int) % 2 == 0) >>== (1 to 10).toStream) apply(_ => List())) assert_=== List(2, 4, 6, 8, 10)

  ((collect[Unit, List[Int], List] %= group(3) >>== (1 to 9).toStream) apply(_ => List())) assert_=== List(List(1, 2, 3), List(4, 5, 6), List(7, 8, 9))
  ...
  ((m1 >>== Stream(1,2,3)) apply(_ => none)) assert_=== Some(1 -> 2)
  ...
  ((colc >>== r) map(_ flatMap (_.toOption)) apply(_ => IO(List())) unsafePerformIO) assert_=== List('f', 'i', 'l', 'e')

  val take10And5ThenHead = take[Unit, Int, List](10) zip take[Unit, Int, List](5) flatMap (ab => head[Unit, Int, Id] map (h => (ab, h)))
  (take10And5ThenHead >>== (1 to 20).toStream) apply(_ => ((Nil, Nil), None)) assert_=== (((1 to 10).toList, (1 to 5).toList), Some(11))
}